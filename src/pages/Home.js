    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <section className="relative w-full h-screen">
        <Slider {...sliderSettings} className="absolute top-0 left-0 w-full h-full">
          {[Kabaka4, Kabaka3, bug10, bug1, bug4, bug5].map((image, index) => (
            <div key={index} className="w-full h-screen flex justify-center items-center relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center bg-black bg-opacity-50 p-6">
                <h1 className="text-6xl font-bold">Tukwanirizza ku Buganda Yiyo Buganda Yange</h1>
                <p className="mt-4 text-xl">Empowering the youth of Buganda, celebrating our culture.</p>
                <Link to="/#hero" className="mt-6 px-10 py-4 bg-yellow-500 text-black rounded-lg text-xl font-semibold hover:bg-yellow-600 transition-all flex items-center justify-center space-x-2">
